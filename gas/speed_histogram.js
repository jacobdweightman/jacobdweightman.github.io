...
function histogramData() {
    ...
    for (let p of particles) {
        ...
        let bucket = Math.floor(V_BUCKET_WIDTH_RATIO * speed / V);
...
function renderHistogram() {
    ...
    for (let i = 0; i < NUM_BUCKETS; i++) {
        c.fillRect(W + W * i / NUM_BUCKETS, H, W / NUM_BUCKETS, -2 * H * data[i] / N);
    }

    c.strokeStyle = "#F00";
    c.lineWidth = 5;
    c.beginPath(W, H);
    for (let i = 0; i < 3 * NUM_BUCKETS; i++) {
        let vByV0 = i / (3 * V_BUCKET_WIDTH_RATIO);
        c.lineTo(W + W * i / (3 * NUM_BUCKETS), H - H * vByV0 * Math.exp(-(vByV0**2)));
    }
    c.stroke();
}
